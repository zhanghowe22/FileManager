...
CFileView::CFileView(QWidget* parent) :
    ...
    connectButton(new QPushButton(QStringLiteral("连接"), this)),
    refreshButton(new QPushButton(QStringLiteral("刷新文件列表"), this)), fileList(new QListWidget(this)),
    downloadButton(new QPushButton(QStringLiteral("下载文件"), this)),
    deleteButton(new QPushButton(QStringLiteral("删除文件"), this)), connectionStatusLabel(new QLabel(this)) {
    // 设置UI布局
    setupUI();
    // 设置控件样式
    setupStyles();
...
void CFileView::showDownloadProgress(const QString& fileName, int progress) {
    QMessageBox::information(this, QStringLiteral("下载进度"),
                             QString("文件 %1 下载完成 %2%").arg(fileName).arg(progress));
}

void CFileView::showError(const QString& message) { QMessageBox::critical(this, QStringLiteral("错误"), message); }
...
void CFileView::setupUI() {
    ...
    connectionLayout->addWidget(new QLabel(QStringLiteral("IP 地址:"), this));
    connectionLayout->addWidget(ipInput);
    connectionLayout->addWidget(new QLabel(QStringLiteral("端口:"), this));
...
void CFileView::setupStyles() {
    ...
    refreshButton->setStyleSheet(
        "background-color: #ffffff; border: 1px solid #ddd; border-radius: 5px; padding: 8px;");
    ...
    // 连接状态标签样式
    connectionStatusLabel->setText(QStringLiteral("未连接"));
    connectionStatusLabel->setStyleSheet("color: #FF0000; font-weight: bold; font-size: 16px;");
}
